...
▶ **Play it live: [instagib.win](https://instagib.win)** — no download, no install.
...
- **Progression.** XP / levels / credits bound to an optional account (guest progress stays local), end-of-match XP + credits roll-up, daily/weekly **challenges**, and a first-win-of-the-day bonus.
...
require agreeing to the lightweight [Contributor License Agreement](CLA.md) in
the pull request template. Found a bug or have an idea? Use the in-game
**Send feedback** button (it lands in the admin panel) or
...
Original design rationale (some of it aspirational and ahead of the current
implementation) lives in [`docs/instagib-arena-plan.md`](docs/instagib-arena-plan.md).
